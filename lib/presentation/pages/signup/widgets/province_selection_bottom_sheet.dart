...
class ProvinceSelectionBottomSheet extends StatelessWidget {
            ...
            return Column(
              children: [
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text('Chọn tỉnh/thành phố', style: AppTextStyles.h2),
                                SizedBox(height: 4.h),
                                Text(
                                  uiState.selectedProvince != null
                                      ? 'Đã chọn: ${uiState.selectedProvince!.name}'
                ...
                Expanded(
                  child: Builder(
                    builder: (context) {
                      // Find the selected province index for initial scroll position
                      int? selectedIndex;
                      if (uiState.selectedProvince != null) {
                        selectedIndex = availableProvinces.indexWhere(
                          (province) => province.provinceId == uiState.selectedProvince!.provinceId,
                        );
                      ...
                      return ListView.separated(
                        ...
                        itemCount: availableProvinces.length,
                        separatorBuilder: (context, index) => SizedBox(height: 8.h),
                        itemBuilder: (context, index) {
                          final province = availableProvinces[index];
                          final isSelected = uiState.selectedProvince?.provinceId == province.provinceId;

                          return GestureDetector(
                            onTap: () {
                              context.read<SignupBloc>().add(SignupProvinceChanged(province));
